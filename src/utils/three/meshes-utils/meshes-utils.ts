import IfcItem from '@/classes/ifc-item'
...
import { BufferAttribute, BufferGeometry, Color, Matrix4, MeshLambertMaterial } from 'three'
 ...
 */
const buildIfcItem = (ifcAPI: IfcAPI, modelID: number, flatMesh: FlatMesh, model: IfcModel): IfcItem => {
	const { geometries, expressID } = flatMesh

	const ifcItem = new IfcItem(expressID)
	ifcItem.userData.expressId = expressID
	...
	for (let index = 0; index < geometries.size(); index++) {
		...
		let geometry = model.getItemGeometry(geometryId)
		...
		let material = model.getItemMaterial(materialId)
		...
		if (!material) {
			...
			model.setItemMaterial(materialId, material)
		}
